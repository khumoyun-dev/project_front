...
const useUpdateUserByAdmin = (selectedUser, editActions) => {
    ...
    const editActionsForAdmin = [
        {
            id: '3',
            title: `${selectedUser && selectedUser?.roles.includes('admin')
                ? `${t('userCollector')}`
                : `${t('userAdmin')}`
                }`,
        ...
        {
            id: '4',
            title: `${selectedUser && selectedUser?.isBlocked
                ? `${t('userUnblock')}`
                : `${t('userBlock')}`
                }`,
